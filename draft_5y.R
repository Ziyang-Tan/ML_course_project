...
# try lasso with glmnet
# model.lasso <- train(
#   Outcome ~., data = train.data, method = "svmLinear",
#   na.action = na.pass,
#   trControl = trainControl("cv", number = 10, search = 'grid'),
#   preProcess = c("center","scale"),
#   tuneGrid = expand.grid(C = seq(0, 2, length = 20),.alpha=1, .lambda = seq(0,100,by = 0.1)),
#   metric = 'Accuracy',
#   family = 'binomial'
# )
# print(model.lasso)
# predicted.lasso <- model.lasso %>% predict(test.data)
# mean(predicted.lasso == test.data$Outcome)
